function renderPopupManagerSummary (parent, managerName, year) {
    ...
    `;
    popupCompilationContainer.appendChild(managerImgContainer);
    ...
    managerContent.innerHTML = `
        ...
        <p class="managerStats">🎵 Gigs bokade: ${summary.totalGigs}</p>
        <p class="managerStats">💰 Totala intäkter: ${summary.totalIncome}</p>
        <p class="managerStats">🎤 DJs bokade: ${summary.bookedDJs}</p>
        <p class="managerStats">👥 Publik: ${summary.totalAttendees}</p>
        <p class="managerStats">⚖️ Genomsnittlig inkomst/gig: ${totalGigs / totalIncome}</p>
    `;
    popupCompilationContainer.appendChild(managerContent);
}